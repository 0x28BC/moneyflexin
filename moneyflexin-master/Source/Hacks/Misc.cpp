...
void Misc::watermark(const Memory& memory) noexcept
{
    ...
    ImGuiWindowFlags windowFlags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove;
    if (!gui->isOpen())
        windowFlags |= ImGuiWindowFlags_NoInputs;
    ...
    ImGui::SetNextWindowPos({ screenX - 270.f , 50.f });
    ...
    ImGui::Text("moneyflexin v1 | %d fps | %d ms | %s", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency(), tm_str);
    ImGui::End();
}
